...
// ── Live stock price cache — refreshes from Yahoo every 30s ──
const priceCache = {};   // { AAPL: { price, change, changePct, ts } }
const https = require('https');
const zlib  = require('zlib');

function fetchFromYahoo(tickers) {
  return new Promise((resolve) => {
    const syms = tickers.join('%2C');
    const tryHost = (host, cb) => {
      const opts = {
        hostname: host,
    ...
    tryHost('query1.finance.yahoo.com', (err, data) => {
      if (!err && data?.quoteResponse?.result?.length) return resolve(data);
      tryHost('query2.finance.yahoo.com', (err2, data2) => {
        resolve((!err2 && data2) ? data2 : null);
      });
    });
  });
}
...
async function refreshPriceCache(tickers) {
  try {
    const data = await fetchFromYahoo(tickers);
    if (!data) return;
...
app.get('/api/stocks', requireAuth, async (req, res) => {
  ...
  // Refresh cache if any ticker is missing or stale (> 30s old)
  const stale = tickers.filter(t => {
    const c = priceCache[t];
    return !c || (Date.now() - c.ts) > 30000;
  });
  if (stale.length) await refreshPriceCache(tickers);
